...
function main(request, response) {
	...
	Parse.Cloud.httpRequest({
		...
		success: function(httpResponse) {
			...
			RESPONSE.tags = JSON.parse(responseStr).tags;
	...
	query.get("lDVBFtrfAz", {
	  	...
	  	success: function(parseBlogInfo) {
	    	if (bloggerHasUpdated(bloggerInfo, parseBlogInfo)) {
	    		...
	    		RESPONSE.response.success("No update needed");
	    	}		 
	  	},
	  	
	  	error: function(object, error) {
	    	RESPONSE.response.error("Fetch parse blog info failed: " + error.code + ", " + error.message);	
	  	}
	...
	parseBlogInfo.save(
		{
        	success: function(savedParseBlogInfo) {
            	RESPONSE.trySendSuccess("blogInfo");
            },
            error: function(object, error) {
                RESPONSE.response.error("Save parse blog info failed: " + error.code + ", " + error.message);
            }
        }
    );
}
		...
		success: function(httpResponse) {
			...
			query.find({
			    ...
			    error: function(error) {
			        RESPONSE.response.error("Fetch parse post headers failed: " + error.code + ", " + error.message);
			    }
			});
...
function updateParsePosts(bloggerPostHeaders, parsePostHeaders) {
	...
	if (deletedPosts.length == 0
		...
		RESPONSE.response.success("No post update necessary, updated blog info only");
	}

	RESPONSE.taskTotals.deleted = deletedPosts.length;
	RESPONSE.taskTotals.updated = updatedPosts.length;
	RESPONSE.taskTotals.saved = newPostIds.length;
	...
	for (var i=0; i<updatedPosts.length; i++) {
		...
		Parse.Cloud.httpRequest({
			...
			error: function(httpResponse) {
				RESPONSE.response.error("Fetch blogger post (bloggerId=" + updatedPosts[i].get("bloggerId") + ") failed: " + httpResponse.status);
			}
	...
	for (var i=0; i<newPostIds.length; i++) {
		...
		Parse.Cloud.httpRequest({
			...
			error: function(httpResponse) {
				RESPONSE.response.error("Fetch blogger post (bloggerId=" + newPostIds[i] + ") failed: " + httpResponse.status);
			}
	...
	parsePost.destroy(
		{
  			success: function(deletedParsePost) {
    			RESPONSE.trySendSuccess("deleted");
  			},
  			error: function(parsePost, error) {
				RESPONSE.response.error("Delete parse post (parseId=" + parsePost.id + ") failed: " + error.code + ", " + error.message);
  			}
  		}
	);
}
			...
			parsePost.save(
				{
		        	success: function(updatedParsePost) {
		            	RESPONSE.trySendSuccess("updated");	            	
		            },
		            error: function(parsePost, error) {
		                RESPONSE.response.error("Update parse post (parseId=" + parsePost.id + ") failed: " + error.code + ", " + error.message);
		       		}
		       	}
		    );
		},
		error: function(category, error) {
			RESPONSE.response.error("Retrieve category " + categoryName + " failed: " + error.code + ", " + error.message);
		}
	});
...
function savePost(bloggerPost) {
	...
	query.first({
		success: function(category) {
		    ...
		    acl.setRoleWriteAccess(RESPONSE.moderatorRole, true);
		    post.setACL(acl);
			...
			post.save(
				{
					...
					success: function(savedParsePost) {
						RESPONSE.trySendSuccess("saved");
					},
					error: function(parsePost, error) {
		                RESPONSE.response.error("Save parse post (parseId=" + parsePost.id + ") failed: " + error.code + ", " + error.message);
					}
				}
			);		
		},
    ...
    if (bloggerCategories !== undefined) {
        ...
        for (var i=0; i<bloggerCategories.length; i++) {
            ...
            for (var j=0; j<RESPONSE.tags.length; j++) {
                if (bloggerCategory.toString() == RESPONSE.tags[j].id_category.toString()) {
                    categories.push(RESPONSE.tags[j].name);
                }
            }
        ...
        for (var i=0; i<RESPONSE.tags.length; i++) {
            if (author.toString() == RESPONSE.tags[i].id_author.toString()) {
        		categories.push(RESPONSE.tags[i].name);
            }
        }
